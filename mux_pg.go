...
func insertUser(user User) int64 {
	...
	sqlStatement := `INSERT INTO people (id,name, Address, age) VALUES ($1, $2, $3,$4) RETURNING id`
	...
	err := db.QueryRow(sqlStatement, user.ID, user.Name, user.Address, user.Age).Scan(&id)
...
func getUser(id int64) (User, error) {
	...
	err := row.Scan(&user.ID, &user.Name, &user.Address, &user.Age)
...
func getAllUsers() ([]User, error) {
	...
	for rows.Next() {
		var user User
		err = rows.Scan(&user.ID, &user.Name, &user.Address, &user.Age)
...
func updateUser(id int64, user User) int64 {
	db := createConnection()
	defer db.Close()
	sqlStatement := `UPDATE people SET name=$2, Address=$3, age=$4 WHERE id=$1`

	res, err := db.Exec(sqlStatement, id, user.Name, user.Address, user.Age)
